...
* Use the Docker Desktop for Windows to create docker containers with tensorflow [Currently GPU is not supported, hence the next option]
* Use Windows Subsystem for Linux (WSL)

I decided to go with the last option. And it was a good timing as Microsoft had recently announced the support for GPU acceleration in WSL 2. So, the plan is as follows :
...
There are many online resources that explain how to do this and that I took help from. I have listed them at the end of the post.

Please note that as of 26th Jun 20, most of these features are still in development phase. They worked for me and you may try them at your own risk, as they did end up messing some parts of my system. You will need to sign up for below two programs :

* [Windows Insider](https://insider.windows.com/en-us/)
* [NVIDIA Developer Program](https://developer.nvidia.com/developer-program)

Also, at each stage I have mentioned some `CHECKS` that would help in making sure you are on the right track.

### Windows Insider Preview Build 20150

First and foremost the GPU support on WSL is not available in the Retail Build of Windows 10. I had to get the latest [Windows 10 Insider Preview Build 20150](https://blogs.windows.com/windowsexperience/2020/06/17/announcing-windows-10-insider-preview-build-20150/). The details of all the builds are available in the [Flight Hub](https://docs.microsoft.com/en-us/windows-insider/flight-hub/). The process took about an hour. _Later on, I realised that my Ubuntu Dual boot went missing, and I suspect it must have happened at this stage_
...
*As per my understanding, in the future releases both the installation of WSL and Linux Kernel will be handled by these two commands :  `wsl --install` and `wsl --update`*
...
* In the end, I went ahead with Ubuntu 18.04 (I might be wrong but I have read somewhere that tensorflow has issues with Ubuntu 20.04)
...
### Install CUDA drivers on Win 10 (host)
...
* Enter the WSL by typing `wsl` on PS/CMD 
...
**CHECK : Docker version** can be found using `docker -v`


### Install NVIDIA Container Toolkit

Below commands will do the installation. [Source : CUDA on WSL](https://docs.nvidia.com/cuda/wsl-user-guide/index.html#setting-containers)
```
...
If it doesnt work you could try `sudo systemctl restart docker`


**CHECK**

* This will create docker container to test whether the installation was sucessful
    + `$ docker run --gpus all nvcr.io/nvidia/k8s/cuda-sample:nbody nbody -gpu -benchmark`
* Some additional test examples are also [available](https://docs.nvidia.com/cuda/wsl-user-guide/index.html#setting-containers)
...
* The CUDA toolkit doesn't seem to work on Debian distro
* The Docker Desktop for Windows should not be running as it interferes with the docker daemon in Ubuntu
  + Stop the Docker Desktop
  + If your `wsl -l -v` command shows `docker-desktop` and `docker-desktop-data`, then the same can be deregistered using `wslconfig /u docker-desktop`
* Some other useful `wsl` options
...
This was just a start and I could run into issues as I continue using tensorflow. But in summary, it was a good learning experience even though I ended up spending couple of days in setting it up.  I am not an expert in Docker neither in CUDA, so if you find any issues in the blog please leave a comment. You can also reach out to the NVIDIA team, they are highly engaged and promptly reply to queries on their forums. Cheers !
